...
export class CreateDirectdebitComponent implements OnInit {
  ...
  onSubmit() {
   ...
   this.directDebitService.directDebitsList.forEach((element:any) => {
     console.log(element.name!=this.directDebit.value.name);
     
     element.name!=this.directDebit.value.name&&
      element.sum!=this.directDebit.value.sum&&
      element.date!=this.directDebit.value.date?this.exist=true:false
